...
impl Cache {
    ...
    fn get_cache_dir() -> Result<PathBuf, MyError> {
        ...
        std::fs::create_dir_all(&cache_dir).map_err(Cache::make_cache_error)?;
    ...
    pub(crate) fn get_from_data_dir(fname: &str) -> Result<PathBuf, MyError> {
        ...
        match file_path.exists() {
            true => Ok(file_path),
            false => Err(MyError::CacheError(format!(
...
mod tests {
    ...
    fn test_cache() {
        let r =
            Cache::cache_text_from("https://danielruss.github.com/codingsystems/soc2010_all.csv");
        assert!(r.is_ok());
        let r =
