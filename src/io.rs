...
use crate::{
    classifier::{JobDescription, ModelConfig, ModelType},
    crosswalk::{Crosswalk, KnownClassificationSystem, KnownCrosswalk, Resolve},
    error::SoccerError,
};
...
where
    I: Iterator<Item = Result<S, SoccerError>>,
    M: JobMapper<S>,
    S: ErrorRecord,
{
    records: I,
...
where
    ...
    S: ErrorRecord,
{
    type Item = (Result<JobDescription, SoccerError>, S);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let raw_record = match self.records.next()? {
            ...
            Err(e) => {
                let msg = match &e {
                    SoccerError::CSVSyntaxError { source, line } => {
                        format!("CSV Syntax error on line {}: {}", line, source)
                    }
                    _ => format!("CSV Syntax error on unknown line {}", e),
                };
...
impl CSVSchema {
    pub(crate) fn from_headers(
        ...
        let text1_col = text1_col.ok_or_else(|| {
            SoccerError::BuilderError(format!("CSV missing required column '{}'", text1_column))
        })?;
